...
class Upsampler_3(nn.Module):
    def __init__(self, scale, in_channels, out_channels, kernel_size=3, bias=True, bn=False, act='relu'):
        ...
        self.F = 64
        self.conv_1 = nn.Conv1d(in_channels, self.F * 4, kernel_size, padding=(kernel_size//2), bias=bias)        
        self.PixelShuf_1 = PixelShuffle(2)
    ...
    def forward(self, x, HH = 32):
        ...
        x = rearrange(x, "B F H W -> B H F W", H=HH*2)
        x = rearrange(x, "B H F W -> (B H) F W", H = HH*2)
...
class Model(nn.Module):
    ...
    def __init__(self,) -> None:
        ...
        self.upsampler = Upsampler_3(scale=2, in_channels=self.num_of_features, out_channels=self.num_of_features)
    ...
    def get_line_from_cache(self, inference_params, initialize_states=0, batch_size=1, height=1,  channels = 202, width = 64):
            if initialize_states == 0:
    ...
    def forward(self, xx, inference=False, index_initialize_states=0):   
        ...
        if inference is False:
            ...
            for i in range(height - 1):
                ...
                slice_2 = xx[:, :, i:i+2, :]
                # Upsample the slice
                upsampled_1=F.interpolate(slice_2,size=(3,width*2), align_corners=False, mode='bilinear')
                upsampled = upsampled_1[:,:,:2,:]
        ...
        if inference is True:
            ...
            upsampled_1=F.interpolate(two_lines,size=(3,width*2), align_corners=False, mode='bilinear')
            bilinear_upsampling = upsampled_1[:,:,:2,:]
