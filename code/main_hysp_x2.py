...
from config_x2 import Config
from final_model_hysp_x2 import Model
...
def main(rank, world_size,train_dataset, val_dataset, test_dataset):
    ...
    #-------------------------------------------------------Logging-------------------------------------------------------#
    log_dir =os.path.join(config.log_dir,'DPSR_Hyspecnetx2_'+model_time)
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        for step, x_hr in enumerate(tqdm(train_dataloader, leave=False)):            
           ...
           #------------------------------------------------DownsamplingByFactor2------------------------------------------------# 
            with torch.no_grad():            
                x_lr = F.interpolate(x_hr, scale_factor=1/2, mode='bicubic', align_corners=False, antialias=True)                
                x_lr = x_lr.to(config.device)                
            #------------------------------------------------ImageSuperRes------------------------------------------------#            
            x_sr = model(x_lr)    
            #------------------------------------------------Loss------------------------------------------------#            
            x_hr_senza_ultime_2_linee = rearrange(x_hr, "B C H W-> B C W H")            
            x_hr_senza_ultime_2_linee = x_hr_senza_ultime_2_linee[:, :, :, :-2]            
            x_hr_senza_ultime_2_linee = rearrange(x_hr_senza_ultime_2_linee, "B C W H-> B C H W")         
            loss= h_loss(x_sr, x_hr_senza_ultime_2_linee)/ accumulation_steps       
            loss.backward()            
        ...
        if epoch % config.validate_every == 0:
                ...
                with torch.no_grad():                    
                    ...
                    for val_step, x_hr1_val in enumerate(tqdm(val_dataloader, leave=False)):
                        ...
                        x_lr_val = F.interpolate(x_hr1_val, scale_factor=1/2, mode='bicubic', align_corners=False, antialias=True)                        

                        x_sr_val = model(x_lr_val)
                        
                        x_sr_val = torch.clamp(x_sr_val, 0, 1)                       
                        x_hr1_val = rearrange(x_hr1_val, "B C H W-> B C W H")            
                        x_hr1_val = x_hr1_val[:, :, :, :-2]                        
                    ...
                    if global_psnr_val > max_psnr:
                        ...
                        if rank == 0:
                            ...
                            if not os.path.exists(checkpoint_dir):
                                os.makedirs(checkpoint_dir)
                            model_name = 'DPSR_Hyspecnetx2_best'
                            checkpoint_path = f'{checkpoint_dir}/epoch_{epoch}_model_{model_name}.pth'
                            torch.save(checkpoint, checkpoint_path)
        ...
        if epoch == config.epochs - 1:
                ...
                with torch.no_grad():                    
                    ...
                    for test_step, x_hr1_test in enumerate(tqdm(test_dataloader, leave=False)):
                        ...
                        x_lr_test = F.interpolate(x_hr1_test, scale_factor=1/2, mode='bicubic', align_corners=False, antialias=True)                        
